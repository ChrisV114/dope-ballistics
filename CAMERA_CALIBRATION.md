...
A future session may let the user pre-mark and order stationary targets before shooting. With the phone fixed, a configured and confirmed shot count may switch the preview to the next saved digital crop. It cannot pan, select an unmarked target, identify an impact, infer a hit, calculate a correction, show an aim point or run Range Analyst while the string is active. Low-confidence counts pause for manual confirmation; impact review stays post-string.
